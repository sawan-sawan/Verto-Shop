// src/components/Men.jsx (FINAL & COMPLETE CODE - ANIMATION REMOVED)

import React, { useState, useEffect, useMemo, useRef, useCallback } from "react";
import "../pages/Men.css"; // आपका CSS फाइल
    ...
    return (
        <div className="men-catalog-popup-overlay" onClick={onClose}>
            <div className="men-catalog-popup-content product" onClick={handlePopupContentClick}>
                <button className="men-catalog-popup-close-btn" onClick={onClose}><CloseIcon /></button>
                <div className="men-catalog-popup-image"><img src={product.images[0]} alt={product.name} /></div>
                <div className="men-catalog-popup-details">
                    <h2>{product.name}</h2>
                    <p className="men-catalog-popup-price">${product.price.toFixed(2)}</p>
                    <div className="men-catalog-size-selector">
                        <label>Size</label>
                        <div className="men-size-options">
                            {sizes.map(size => (<button key={size} onClick={() => setSelectedSize(size)} className={`men-size-btn ${selectedSize === size ? 'selected' : ''}`}>{size}</button>))}
                        </div>
                    </div>
                    <div className="men-quantity-selector">
                    ...
                    <div className="men-popup-actions">
                        <button onClick={handleAddToCartClick} className="men-popup-add-to-cart-btn">Add to cart</button>
                        <button className="men-popup-buy-now-btn">Buy with VertoShop</button>
                        <a href="#" className="men-more-payment-options">More payment options</a>
                    </div>
                </div>
            </div>
        </div>
...
const LoginRequiredPopup = ({ onClose, onNavigate }) => {
    ...
    return (
        <div className="men-catalog-popup-overlay" onClick={onClose}>
            <div className="men-catalog-login-content" onClick={handlePopupContentClick}>
                <button className="men-catalog-popup-close-btn" onClick={onClose}><CloseIcon width={22} height={22} /></button>
                <div className="men-catalog-login-image"><LoginIllustrationIcon /></div>
                <div className="men-catalog-login-details">
                    <h2>Please Login or Sign Up</h2>
                    <p>You need to log in before adding items to your cart.</p>
                    <button onClick={onNavigate} className="men-catalog-login-nav-btn">Go to Login / Sign Up</button>
                </div>
            </div>
        </div>
    );
...
// --- Main Men Component ---
const Men = ({ onAddToCart, currentUser, justAddedProductId }) => {
    ...
    return (
        <div className="men-catalog-page-container">
            <header className="men-catalog-header"><h1>Men's Collection</h1></header>

            <div className="men-catalog-toolbar">
                <div className="men-catalog-filters" ref={filterRef}>
                    <div className="men-catalog-filter-group">
                        <button onClick={() => toggleFilter('availability')} className="men-catalog-filter-btn">
                            Availability <ChevronDownIcon />
                        </button>
                        {openFilter === 'availability' && (
                            <div className="men-catalog-filter-dropdown">
                                <ul className="men-catalog-dropdown-list">
                    ...
                    <div className="men-catalog-filter-group">
                        <button onClick={() => toggleFilter('color')} className="men-catalog-filter-btn">
                            Color <ChevronDownIcon />
                        </button>
                        {openFilter === 'color' && (
                            <div className="men-catalog-filter-dropdown">
                                <div className="men-catalog-color-list">
                                    {colorOptions.map(color => (
                                        <button key={color} className={`men-catalog-color-swatch ${activeFilters.color.includes(color) ? 'selected' : ''}`} style={{ backgroundColor: color.toLowerCase() }} onClick={() => handleFilterChange('color', color)} title={color} />
                                    ))}
                                </div>
                            </div>
                        )}
                    ...
                    <div className="men-catalog-filter-group">
                        <button onClick={() => toggleFilter('category')} className="men-catalog-filter-btn" disabled={subCategoryOptions.length === 0}>
                ...
                <div className="men-catalog-tools">
                    <span className="men-catalog-item-count">{filteredAndSortedProducts.length} items</span>
                    <div className="men-catalog-filter-group">
                        <button onClick={() => toggleFilter('sort')} className="men-catalog-filter-btn">
                            Sort <ChevronDownIcon />
                        </button>
                        {openFilter === 'sort' && (
                            <div className="men-catalog-filter-dropdown men-catalog-sort-dropdown">
                                <ul className="men-catalog-dropdown-list men-catalog-sort-list">
            ...
            <div className={`men-catalog-grid ${gridLayout}`}>
                {filteredAndSortedProducts.map(product => (
                    <div key={product.id} className="men-catalog-card">
                        <div className="men-catalog-img-wrapper">
                            <img src={product.images[0]} alt={product.name} className="men-catalog-img-main" />
                            {product.images[1] && <img src={product.images[1]} alt={product.name} className="men-catalog-img-hover" />}
                        </div>
                        <div className="men-catalog-info">
                            <p className="name">{product.name}</p>
                            <p className="price">${product.price}</p>
                            <button className="men-catalog-add-btn" onClick={() => handleOpenPopup(product)}>
                                <PlusIcon /> Add
                            </button>
                        </div>
                    </div>
